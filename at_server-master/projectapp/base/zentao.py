...
class Zentao:
    ...
    def login(cls, account):
        result = requests.post(f'{host}/user-loginbytms.json',
                               data={'account': account, 'sign': 'tms!@!#!asADAD@#'})
        if result.json().get('data'):
            return result.cookies.get('zentaosid')
        else:
            return None
    ...
    @classmethod
    def get_story(cls, zentaosid, id):
        cookies = dict(zentaosid=zentaosid)
        result = requests.get(f'{host}/story-view-{id}-0.json', cookies=cookies)
        try:
            return result.json()
        except:
            return None
